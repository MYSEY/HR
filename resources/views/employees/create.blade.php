                        ...
                        <div id="PermanentAddress">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label>Province/City</label>
                                        <select class="form-control" @change="perCityChange" name="permanent_addr_city" v-model="city" :disabled="JSON.stringify(Percities).length==2" value="{{old('city')}}">
                                            <option v-for="(item,text) in Percities" :value="text">@{{item}}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label>District/Khan</label>
                                        <select class="form-control" @change="perDistricChange" name="permanent_addr_distric" v-model="distric" :disabled="JSON.stringify(Perdistrics).length==2" value="{{old('distric')}}">
                                            <option v-for="(item, text) in Perdistrics" :value="text">@{{item}}</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="form-group col-md-6">
                                    <label class="no-error-label">Commune/Sangkat</label>
                                    <select class="form-control no-error-border" @change="perCommuneChange" name="permanent_addr_commune" v-model="commune" :disabled="JSON.stringify(Percommunes).length==2" value="{{old('commune')}}">
                                        <option v-for="(item, text) in Percommunes" :value="text">@{{item}}</option>
                                    </select>
                                </div>
                                <div class="form-group col-md-6">
                                    <label class="no-error-label">Village</label>
                                    <select class="form-control no-error-border" @change="perVillageChange" name="permanent_addr_village" v-model="village" :disabled="JSON.stringify(Pervillages).length==2" value="{{old('village')}}">
                                        <option v-for="(item, text) in Pervillages" :value="text">@{{item}}</option>
                                    </select>
                                </div>
                            </div>
                        </div>
    ...
    window.onload = function () {
        ...
        var main = new Vue({
            el: '#PermanentAddress',
                ...
                methods:{
                    perCityChange: async function(){
                        var me = this;
                        this.hidden = this.Percity;
                        await this.PergetData(this.Percity).then(function(response){
                        me.Perdistrics = response.data;
                        me.Percommunes={};
                        me.Pervillages={};
                    });
                },
                perDistricChange: async function(){
                    var me = this;
                    this.hidden = this.Perdistric;
                    await this.PergetData(this.Perdistric).then(function(response){
                        me.Percommunes = response.data;
                        me.Pervillages={};
                    });
                },
                perCommuneChange: async function(){
                    var me = this;
                    this.hidden = this.Percommune;
                    await this.PergetData(this.Percommune).then(function(response){
                        me.Pervillages = response.data;
                    });
                },
                perVillageChange:function(){
                    this.hidden = this.Pervillage;
                },
                PergetData:function(code=''){
                    if(code){ 
                        return axios.get('{{route("permanent.address")}}?code='+code)
                    }
                    else
                    { 
                        return axios.get('{{route("permanent.address")}}')
                    }
                }
            },
            ...
            created: async function(){
                var me = this;
                this.PergetData().then(function(response){
                    me.Percities = response.data;
                    console.log(me.Percities);
                });
            }
        });
    }
