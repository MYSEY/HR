...
@section('script')
    <script>
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            ...
                            `;
                            if ((userIdLog == data.employee_id && (row.status =="new" || row.status =="4")) || (userPermission.is_update == 1  && (userIdLog == data.review_employee_id && (row.status =="new" || row.status =="1")))){
                                actionHtml += `
        ...
        function updateTopProgressStepper(status) {
            // បង្កើតលំនាំដើមសម្រាប់ Step ទាំង ៤ និងខ្សែតភ្ជាប់ (Line) ទាំង ៣
            ...
            let step4Class = "",       step4Content = "4";

            // 🎯 ឆែក Condition ឡើងវិញបន្ទាប់ពីដក Accepted ចេញ
            if (status === 'new' || status === 'preparing' || status === '4') {
                // ជំហានទី ១: Prepared (ករណីបង្កើតថ្មី កំពុងរៀបចំ ឬត្រូវបាន Return [status 4] មកវិញ)
                step1Class = "active"; step1Content = "1"; line1Class = "half";
            } 
            else if (status === '1') {
                // ជំហានទី ២: Reviewed (ផ្អែកលើ DataTable: '1' = Pending Review)
                step1Class = "completed"; step1Content = "✓"; line1Class = "full";
                step2Class = "active";    step2Content = "2"; line2Class = "half";
            } 
            else if (status === '2') {
                // ជំហានទី ៣: Verified by HR (ផ្អែកលើ DataTable: '2' = Pending Verify)
                step1Class = "completed"; step1Content = "✓"; line1Class = "full";
                ...
                // ជំហានទី ៤: Approved (ផ្អែកលើ DataTable: '3' = Pending Approve ឬ 'approved')
                step1Class = "completed"; step1Content = "✓"; line1Class = "full";
                step2Class = "completed"; step2Content = "✓"; line2Class = "full";
                step3Class = "completed"; step3Content = "✓"; line3Class = "full";
                
                if (status === 'approved') {
                    step4Class = "completed"; step4Content = "✓";
                } else {
                    step4Class = "active";    step4Content = "4";
                }
            }
            let stepperHtml = `
