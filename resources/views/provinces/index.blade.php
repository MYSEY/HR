    ...
    <div class="">
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col">
                    <h3 class="page-title">@lang('lang.province')</h3>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{url('/dashboad/employee')}}">@lang('lang.dashboard')</a></li>
                        <li class="breadcrumb-item active">@lang('lang.province')</li>
                    </ul>
                </div>
                                        ...
                                        <table class="table table-striped custom-table mb-0 datatable dataTable no-footer btn_trainer" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Company Name: activate to sort column ascending" style="width: 772.237px;">@lang('lang.khaet_name_km')</th>
                                                    <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Name (KH): activate to sort column ascending" style="width: 772.237px;">@lang('lang.khaet_name_latin')</th>
                                                    <th class="sorting" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Name (EN): activate to sort column ascending" style="width: 772.237px;">@lang('lang.khaet_name_en')</th>
                                                ...
                                                @if (count($data)>0)
                                                    @foreach ($data as $key=>$item)
                                                        <tr class="odd">
                                                            ...
                                                            <td>{{$item->code}}</td>
                                                            <td>{{$item->khaet_name_km}}</td>
                                                            <td>{{$item->khaet_name_latin}}</td>
                                                            <td>{{$item->khaet_name_en}}</td>
                                                            <td>{{$item->name_km}}</td>
...
<script>
    $(function(){
        $("#importProvince").on("click", function() {
            $(".thanLess").hide();
            $("#thanLess").text("");
            $('#importProvinceModal').modal('show');
        });
    });
</script>