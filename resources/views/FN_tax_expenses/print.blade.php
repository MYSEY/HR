<div id="print_tax_expense" hidden>
    <div class="card-header">
        <div>
            <div style="text-align: right" class="font-title">
                <label style="font-size: 16px">Camma-FND-002</label><br>
            </div>
            <div style="text-align: center" class="font-title">
                <label class="title">ខេមា មីក្រូហិរញ្ញវត្ថុ លីមីតធីត</label><br>
                <label style="font-size: 16px">CAMMA MICROFINANCE LIMITED</label><br>
            </div>
            <div style="margin-top: -90px; margin-left: -6%">
                <img style="width:auto;height: 10%;"alt='White' id="image_logo_print"
                    src="http://hrms.camma.com:9090/HR_UAT/public/admin/img/logo/cammalogo.png">
            </div><br>
            <label class="label-subs">ID: <span class="label-subs expense_tracking_id"></span></label><br>
        </div><br>
        <div class="font-sub-title">
            <label class="label-sub">សូមគោរពជូន</label><br>
            <label class="label-sub"><span class="p_kind_regard"></span></label>
        </div><br><br>
        
        <div style="display:flex;" class="set-font">
            <div style="width: 487%;">
                <div class="style-table_">
                    <div>
                        <table style="width:100%">
                            <tr>
                                <td class="table_tr_" style="font-weight: bold; ">កម្មវត្ថុ:</td>
                            </tr>
                        </table>
                    </div>
                    <div>
                        <table style="width:100%">
                            <tr><td class="table_tr" style="word-wrap: break-word; white-space: normal">
                                <span class="p_subject"></span>
                            </td></tr>
                        </table>
                    </div>
                </div>
                <div class="style-table_">
                    <div>
                        <table style="width:100%">
                            <tr>
                                <td class="table_tr_" style="font-weight: bold; ">យោង:</td>
                            </tr>
                        </table>
                    </div>
                    <div>
                        <table style="width:100%">
                            <tr><td class="table_tr">
                                <span class="p_reference"></span>
                            </td></tr>
                        </table>
                    </div>
                </div>
                <table style="width:100%">
                    <tr><td class="table_tr" style="padding-left: 6%;">
                        <span class="p_reason_subject" style="word-wrap: break-word; white-space: normal"></span>
                    </td></tr>
                </table>
                
                {{-- <label class="label-sub_lll">នាយកដ្ឋានដូចខាងក្រោម៖</label> --}}
                <div class="style-table">
                    <table style="width:100%" class="locations_a">
                        <tr >
                        </tr>
                    </table>
                    <table style="width:100%" class="locations_b">
                        <tr>
                           
                        </tr>
                    </table>
                </div>
                <label class="label-sub_lll">ចំណាយរួមមាន៖</label>
                <div class="style-table">
                        <table style="width:100%">
                            <tr>
                                <td class="table_tr_" colspan="2">១ ថ្លៃទំនិញឬសម្ភារ</td>
                            </tr>
                            <tr>
                                <td class="table_tr_" colspan="2">២ ថ្លៃពលកម្ម/ជួល/សេវា/ផ្សេងៗ</td>
                            </tr>
                            <tr>
                                <td class="table_tr_" colspan="2">៣ ចំណាយប្រកាសពន្ធ</td>
                            </tr>
                            <tr>
                                <td class="table_tr_" colspan="2">៤ សរុបចំណាយ (១+២+៣)</td>
                            </tr>
                            <tr class="p_reverse_charge">
                                <td class="table_tr_" colspan="2">៥ អាករជំនួស (VAT Reverse Charge) ១០% </td>
                            </tr>
                            <tr>
                                <td class="table_tr_" colspan="2"><span class="number_supplier"></span></td>
                            </tr>
                            <tr>
                                <td class="table_tr_" colspan="2">- បើកជូនអ្នកផ្គត់ផ្គង់ជាអក្សរ (ដុល្លារអាមេរិក)</td>
                            </tr>
                            <tr>
                                <td class="table_tr_" colspan="2">- បើកជូនអ្នកផ្គត់ផ្គង់ជាអក្សរ (រៀល)</td>
                            </tr>
                            <tr>
                                <td class="table_tr_" colspan="2">- លក្ខខណ្ឌទូទាត់</td>
                            </tr>
                        </table>
                        <table style="width:100%">
                            <tr>
                                <td class="table_tr">: $ <span class="p_ge_cost_material_usd"></span></td>
                                <td class="table_tr">៛ <span class="p_ge_cost_material_riel"></span></td>
                            </tr>
                            <tr>
                                <td class="table_tr">: $ <span class="p_ge_cost_lso_usd"></span></td>
                                <td class="table_tr">៛ <span class="p_ge_cost_lso_riel"></span></td>
                            </tr>
                            <tr>
                                <td class="table_tr">: $ <span class="p_te_tax_usd"></span></td>
                                <td class="table_tr">៛ <span class="p_te_tax_income"></span></td>
                            </tr>
                            <tr>
                                <td class="table_tr">: $ <span class="p_ge_total_cost_usd"></span></td>
                                <td class="table_tr">៛ <span class="p_ge_total_cost_riel"></span></td>
                            </tr>
                            <tr class="p_reverse_charge">
                                <td class="table_tr">: $ <span class="p_vat_reverse_charge_usd"></span></td>
                                <td class="table_tr">៛ <span class="p_vat_reverse_charge_riel"></span></td>
                            </tr>
                            <tr>
                                <td class="table_tr">: $ <span class="p_te_total_usd"></span></td>
                                <td class="table_tr">៛ <span class="p_te_total_tax"></span></td>
                            </tr>
                            <tr>
                                <td class="table_tr" colspan="2">:<span class="p_convertNumberDollar"></span></td>
                            </tr>
                            <tr>
                                <td class="table_tr" colspan="2">:<span class="p_convertNumberRiel"></span></td>
                            </tr>
                            <tr>
                                <td class="table_tr" colspan="2">:<span class="p_payment_term"></span></td>
                            </tr>
                        </table>
                </div>
                <table style="width:100%">
                    <tr><td class="table_tr" >
                        <span id="TEXP_remark" style="margin-left: 4% !important;"></span>
                    </td></tr>
                </table>
                <div style="float: right">
                    <label style="font-size: 16px;">ភ្នំពេញ, ថ្ងៃទី <span class="p_day"></span> ខែ <span class="p_month"></span> ឆ្នាំ <span class="p_year"></span></label>
                </div>
            </div>
        </div><br>
        <div style="display: flex; justify-content: space-between; text-align: center; margin-top: 5px;">
            <!-- Column 1 -->
            <div style="flex: 1;">
              <label class="label-subs" style="display: block; margin-bottom: 60px;"><strong>ឯកភាព</strong></label>
                <label class="label-sub_lll p_approved_by"></label><br>
              <label class="label-sub_lll p_date_approve"></label>
            </div>
            <!-- Column 2 -->
            <div style="flex: 1;">
              <label class="label-subs" style="display: block; margin-bottom: 60px;"><strong>បានត្រួតពិនិត្យ</strong></label>
              <label class="label-sub_lll p_review_by"></label><br>
              <label class="label-sub_lll p_data_review"></label>
            </div>
            <!-- Column 3 -->
            <div style="flex: 1;">
              <label class="label-subs" style="display: block; margin-bottom: 60px;"><strong>រៀបចំដោយ</strong></label>
              <label class="label-sub_lll p_request_by"></label><br>
              <label class="label-sub_lll p_data_request"></label>
            </div>
          
          </div>
    </div>
</div>

