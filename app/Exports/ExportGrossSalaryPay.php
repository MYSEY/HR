...
class ExportGrossSalaryPay implements FromCollection, WithColumnWidths, WithHeadings, WithCustomStartCell, WithEvents
{
    protected $num;
    protected $export_datas;

    protected $totalBasicSalary;
    protected $totalTotalGrossSalary;
    ...
    public function __construct($request)
    {
        ...
        $severance_pay=[];
        $datas = GrossSalaryPay::with("users")
        ->join('users', 'gross_salary_pays.number_employee', '=', 'users.number_employee')
        ->select(
            'gross_salary_pays.*',
        ...
        foreach ($datas as $key => $value) {
            $i++;
            $this->num = $i;
            $this->totalBasicSalary += $value->basic_salary;
            $this->totalTotalGrossSalary += $value->total_gross_salary;
            $this->totalContractSeverancePay += $value->total_fdc1;
            $GrossSalaryPay[]=[
                $i,
        ...
        $this->export_datas = $GrossSalaryPay;
    }
    ...
    public function registerEvents(): array {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $sheet->setCellValue("I".$rows, number_format($this->totalBasicSalary, 2));
                ...
                $sheet->setCellValue("J".$rows, number_format($this->totalTotalGrossSalary, 2));
                ...
                $sheet->setCellValue("K".$rows, number_format($this->totalContractSeverancePay, 2));
